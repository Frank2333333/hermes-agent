...
When a blocked URL is requested, the tool returns an error explaining the domain is blocked by policy. The blocklist is enforced across `web_extract`, `web_extract`, `browser_navigate`, and all URL-capable tools.
