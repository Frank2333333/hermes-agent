...
- It overlaps with Hermes native `web_extract` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Hermes native `web_extract` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
