"""Tests for model_tools.py 鈥?function call dispatch, agent-loop interception, legacy toolsets."""
...
class TestPreToolCallBlocking:
    ...
    def test_invalid_hook_returns_do_not_block(self, monkeypatch):
        """Malformed hook returns should be ignored 鈥?tool executes normally."""
        def fake_invoke_hook(hook_name, **kwargs):
            if hook_name == "pre_tool_call":
                return [
                    "block",
    ...
    def test_skip_flag_prevents_double_block_check(self, monkeypatch):
        ...
        # is not called 鈥?invoke_hook fires directly in the skip=True branch.
