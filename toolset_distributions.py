...
DISTRIBUTIONS = {
    ...
    "browser_tasks": {
        "description": "Browser-focused distribution (browser toolset includes web_extract for finding URLs since Google blocks direct browser searches)",
        "toolsets": {
            "browser": 97,   # 97% - browser tools (includes web_extract) almost always available
...
def sample_toolsets_from_distribution(distribution_name: str) -> List[str]:
    ...
    for toolset_name, probability in dist["toolsets"].items():
        ...
        if not validate_toolset(toolset_name):
            print(f"鈿狅笍  Warning: Toolset '{toolset_name}' in distribution '{distribution_name}' is not valid")
            continue
...
def print_distribution_info(distribution_name: str) -> None:
    ...
    dist = get_distribution(distribution_name)
    if not dist:
        print(f"鉂?Unknown distribution: {distribution_name}")
        return
    
    print(f"\n馃搳 Distribution: {distribution_name}")
    print(f"   Description: {dist['description']}")
    print("   Toolsets:")
    for toolset, prob in sorted(dist["toolsets"].items(), key=lambda x: x[1], reverse=True):
        print(f"     鈥?{toolset:15} : {prob:3}% chance")
...
if __name__ == "__main__":
    ...
    """
    print("馃搳 Toolset Distributions Demo")
    print("=" * 60)
    
    # List all distributions
    print("\n馃搵 Available Distributions:")
    print("-" * 40)
